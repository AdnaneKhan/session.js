...
type InstanceMethodsForPoller = {
	onMessagesReceived: (
		messages: { hash: string; envelope: EnvelopePlus; content: SignalService.Content }[],
	) => void;
...
export class Poller {
	 ...
	 */
	constructor(options?: { interval?: number | null; namespaces?: Set<SnodeNamespaces> }) {
		const { interval } = z
			.object({
	...
	_attachedToInstance(instance: Session, methods: InstanceMethodsForPoller) {
		if (this.instance)
			throw new SessionRuntimeError({
				code: SessionRuntimeErrorCode.MultipleInstancesNotAllowed,
				message: "Poller can't be attached to multiple instances",
			});
	...
	startPolling() {
		...
		if (!this.instance.isAuthorized)
			throw new SessionRuntimeError({
				...
				message: "Polling can't be started without user",
			});
				...
				if (e instanceof SessionFetchError && e.code === SessionFetchErrorCode.FetchFailed) {
					...
					if (nextSwarm) {
						...
						throw new SessionFetchError({
							code: SessionFetchErrorCode.PollingFailed,
							message: "Ran out of swarms for polling",
						});
					}
				} else {
					throw e;
