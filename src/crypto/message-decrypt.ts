...
export function extractContent(message: string): null | Uint8Array {
	try {
		...
		throw new SessionCryptoError({
			...
			message:
				"ExtractWebSocketContent from message failed with" +
...
export function decodeMessage(
	body: Uint8Array,
	options?: { overrideSource: string; ourPubKey: string },
) {
...
export function decryptMessage(keypairs: Keypair[], envelope: EnvelopePlus) {
	if (envelope.content.byteLength === 0) {
		throw new SessionValidationError({
			...
			message: "Received an empty envelope.",
		});
	}
...
export function decryptEnvelopeWithOurKey(keypair: Keypair, envelope: EnvelopePlus): Uint8Array {
	try {
		...
		throw new SessionCryptoError({
			...
			message:
				"Message decryption failed with " +
				(e instanceof Error ? ": " + e.message : " unknown error"),
		});
	}
}
...
export function decryptWithSessionProtocol(
	...
	if (!isValid) {
		throw new SessionCryptoError({
			code: SessionCryptoErrorCode.MessageVerificationFailed,
	...
	if (!senderX25519PublicKey) {
		throw new SessionCryptoError({
			code: SessionCryptoErrorCode.MessageDecryptionFailed,
			message: "Failed to get sender's public key",
		});
	}
