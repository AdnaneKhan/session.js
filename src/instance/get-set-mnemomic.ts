...
export function setMnemonic(this: Session, mnemonic: string, displayName?: string) {
	if (this.isAuthorized)
		throw new SessionRuntimeError({
			code: SessionRuntimeErrorCode.InstanceAlreadyAuthorized,
	...
	if (words.length !== 13)
		throw new SessionValidationError({
			code: SessionValidationErrorCode.InvalidMnemonic,
			message:
				"Invalid number of words in mnemonic provided in setMnemonic call. Expected 13 words, got " +
				words.length,
		});
...
async function loadAvatar(this: Session, avatarStorageRequest: string | Promise<string | null>) {
	const avatarStorageSerialized =
		typeof avatarStorageRequest === "string" ? avatarStorageRequest : await avatarStorageRequest;
