...
export function addPoller(this: Session, poller: Poller) {
	if (!(poller instanceof Poller))
		throw new SessionValidationError({
			code: SessionValidationErrorCode.InvalidPoller,
			message: "Poller must be an instance of Poller",
		});
	...
	poller._attachedToInstance(this, {
		onMessagesReceived: async (messages) => {
			...
			newMessages
				.filter(
					(m) =>
						m.content.dataExtractionNotification &&
			...
			newMessages
				.filter((m) => m.content.callMessage)
