...
export async function sendMessage(
	...
	if (attachments?.some((a) => !(a instanceof File)))
		throw new SessionValidationError({
			code: SessionValidationErrorCode.InvalidAttachment,
			message: "Attachments must be instances of File",
		});
	if (attachments?.some((a) => a.size > MAX_ATTACHMENT_FILESIZE_BYTES))
		throw new SessionValidationError({
			code: SessionValidationErrorCode.InvalidAttachment,
			message: "Attachment size exceeds the limit: " + MAX_ATTACHMENT_FILESIZE_BYTES + " bytes",
		});
	...
	if (attachments) {
		for (const attachment of attachments ?? []) {
			...
			const uploaded = await this._request<ResponseUploadAttachment, RequestUploadAttachment>({
				type: RequestType.UploadAttachment,
				body: { data: encrypted.ciphertext },
			});
	...
	if (voiceMessage) {
		...
		const encrypted = await encryptFileAttachment(
			new File([voiceMessage], filename, { type: voiceMessage.type }),
		);
		const uploaded = await this._request<ResponseUploadAttachment, RequestUploadAttachment>({
			type: RequestType.UploadAttachment,
			body: { data: encrypted.ciphertext },
	...
	const syncMessageHash = await this._storeMessage({
		message: rawSyncMessage,
		data: syncMessageEncrypted,
	});
