		...
		async () => {
			...
			if (!snode)
				throw new SessionFetchError({
					...
					message: "No snodes available",
				});
			try {
				const { swarms } = await this._request<ResponseGetSwarms, RequestGetSwarmsBody>({
					type: RequestType.GetSwarms,
					body: { snode, pubkey: sessionID },
				});
				if (swarms.length === 0) {
					throw new SessionRuntimeError({
						code: SessionRuntimeErrorCode.NoSwarmsAvailable,
						message: "No swarms found for " + sessionID,
					});
				}
				return swarms;
			} catch (e) {
...
export async function getOurSwarm(this: Session) {
	...
	if (!this.ourSwarm)
		throw new SessionRuntimeError({
			code: SessionRuntimeErrorCode.NoSwarmsAvailable,
			message: "No swarms found for this instance",
		});
	return this.ourSwarm;
}
